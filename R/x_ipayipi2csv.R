...
#' @param wanted_tbls Data items/tables that should be exported to csvs.  '%ilike%': ergo seperate multiple search phrases by '|'.
...
ipayipi2csv <- function(
  ...
  if (length(station_files) == 0) {
    cli::cli_abort(c(
      "!" = "No station files found in: {pipe_house$d4_ipip_room}"
    ))
  }
  ...
  exported_stations <- future.apply::future_lapply(
    seq_along(station_files), function(z) {
      ...
      if (!is.null(wanted_tbls)) {
        tbl_names <- tbl_names[tbl_names %ilike% wanted_tbls]
        if (length(tbl_names) == 0 &&
            (verbose == TRUE || xtra_v == TRUE)
        ) {
          cli::cli_inform(c(
            "i" = "In station: {station_files[z]}",
            " " = "No tables matching: {.var wanted_tbls ==} {wanted_tbls}"
          ))
        }
      }
