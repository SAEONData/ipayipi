...
gw_vis_dy <- function(
  ...
  d_main$ut1_level_m <- d_main$ut1_level_m + (mean(d_main$ut1_level_m,
    na.rm = TRUE) - mean(d_main$t_level_m, na.rm = TRUE))
  ...
  if (!is.null(rain_data)) {
    ...
    nomatch = NA)
    d_main1$int <- ifelse(!is.na(d_main1$rain_mm), 1, NA)
    ...
    d_main <- d_main1[, c("id", "Date_time", "ut1_level_m", "level_kpa",
      "temperature_degc", "conduct_mS_per_cm",
      "manual_level_m", "outliers", "drift_neutral", "t_level_m", "rain_mm")]
  } else {
    d_main$rain_mm  <- NA
  }
  ...
  if (vis_type == 1) {
    ...
    if (conduct == TRUE & is.null(rain_data)) seriez <- c(
      seriez, "conduct_mS_per_cm")
    if (!is.null(rain_data)) seriez  <- c(seriez, "rain_mm")
