...
#'  each import into the 'd3_nomvet_room'.
...
#'  1. Create (if none) or produce an inventory of files in the 'd3_nomvet_room'.
#'  1. Compare the inventories to identify which new files should be
#'    transferred into the 'd3_nomvet_room'.
#'  1. Transfer files---saving these in RDS format in the 'd3_nomvet_room'.
#' @param hobo_in Converted hobo rainfall files as produced by
#'  `rain_hobo_conversion()`.
#' @param d3_nomvet_room The directory where the standardised hobo rainfall files
#'  are archived.
...
ipayipi_data_log <- function(
  ...
  methdr <- lapply(slist, function(x) {
    m <- attempt::attempt(expr = readRDS(file.path(log_dir, x)),
      msg = cli::cli_abort("File is corrupt: {file.path(log_dir, x)}"),
      verbose = TRUE
    )
