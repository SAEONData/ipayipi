...
#' @details This function uses `data.table::fread` which is optimized for processing 'big data'. Apart from usual the usual options which can be parsed to `data.table::fread` this function generates some standardised metadata to complement the read from a logger data table (if `data.table::fread()` is unsuccessful `base::read.csv()` is used). This metadata may vary from one logger output to another. To cater for this variation a `data_setup` must be provided. The setup this can be used as a standard for further imports.
#' When importing data a consistency check on the recording interval in the data date-time stamp is performed. A prompt is called if there are inconsistent time intervals between record events, and data rows with inconsistent time intervals will be removed if approved.
...
imbibe_raw_flat <- function(
  ...
  if (is.list(data_setup)) {
    ...
    # if the standard title is the date-time col then return error
    if (head_info_i$station_title %ilike% "date|time") {
      return(list(ipayipi_data_raw = file, err = TRUE, dsi = dsi))
    }
    ...
    if (length(nchk[nchk != FALSE]) == 0 && xtra_v == TRUE) {
