...
#' - `plonk`: Simple rbind(fill = TRUE) (_see_ [base::rbind()]).
...
join_param_eval <- function(
  ...
  p_args <- list(y_phen_names = "NULL", time_seq = TRUE, fuzzy = "NULL",
    full_eval = "NULL", eval_seq = "NULL", f_summary = "NULL", ppsij = "NULL",
    sf = "NULL", inq = "NULL"
  )
  ...
  if (!full_eval) {
    ## partial evaluation -----------------------------------------------------
    # check join types
    j <- list("left_join", "right_join", "inner_join", "full_join", "plonk")
    join <- list(join)
    m <- join[!join %in% j]
