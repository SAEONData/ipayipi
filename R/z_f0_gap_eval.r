...
gap_eval <- function(
  ...
  seq_dt <- lapply(tn, function(x) {
    ...
    dx <- dx[, names(dx)[!names(dx) %ilike% "^uz_|logger|nomvet|origin|format"],
      with = FALSE]
  ...
  e <- ipayipi::sf_read(sfc = sfc, tv = meta_events[1], tmp = TRUE, verbose =
    verbose)[[meta_events[1]]]
  e <- attempt::try_catch(
    .e = ~NULL,
  ...
  if (!is.null(e) && nrow(e) > 0) {
    ...
    e[is.na(event_thresh_s)][
        is.na(start_dttm) | is.na(end_dttm), "event_thresh_s"] <- event_thresh_s
    ...
    e$start_dttm <- data.table::fifelse(e$start_dttm < e$dta_start,
      e$dta_start, e$start_dttm)
    e$end_dttm <- data.table::fifelse(e$end_dttm > e$dta_end,
      e$dta_end, e$end_dttm)
    ...
    iw <- lapply(seq_along(seq_dt), function(i) {
      ...
      man_gaps_r <- data.table::foverlaps(mgi, sq, nomatch = NA,
        mult = "all")
      # define gap type
      ...
      man_gaps_before <- man_gaps_s[ev_start_dttm < gap_start,
      ...
      man_gaps_after <- man_gaps_s[ev_end_dttm > gap_end,
      ...
      mg <- mg[, c("gid", "eid", "table_name", "phen", "gap_type", "gap_start",
      ...
      go <- go[, c("gid", "eid", "table_name", "phen", "gap_type", "gap_start",
      "gap_end", "dt_diff_s", "gap_problem_thresh_s", "problem_gap", "notes")]
