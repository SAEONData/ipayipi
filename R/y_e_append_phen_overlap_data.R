...
append_phen_overlap_data <- function(
  ...
  phen_idx <- lapply(sf_eindx$fs, function(fsi) {
    ...
    if (all(
      ...
      sf_dta_chunkr(dta_room = dta_room, dta_sets = list(ndt), tn = tn,
        ri = ri, rit = rit, chunk_v = chunk_v
      )
      sfo_dttms <- data.table::data.table(
        sfo_min = NULL, sfo_max = NULL
      )
    ...
    if (phen_id) {
      ...
      dold_l <- lapply(dold_l, function(x) {
        ...
        dta <- ipayipi::phen_vars_sts(phen_tbl = ptab, dta_in = x)
        return(dta)
      })
    ...
    if (!is.null(phen_dt)) {
      raw_dto <- ipayipi::phen_vars_sts(dta_in = raw_dto, phen_tbl = phen_dt)
    }
    # save raw_dto to tmp location --- indexed format
    dta_sets <- list(raw_dto)
    names(dta_sets) <- basename(fsi)
    oindxr <- sf_dta_chunkr(dta_room = dta_room, dta_sets = dta_sets,
      tn = tn, ri = ri, rit = rit, chunk_v = chunk_v
    )
    sfo_dttms <- data.table::data.table(
    ...
    if (phen_id) {
      ...
      phen_dtos <- lapply(dto, function(x) {
        ...
        x$ph_int <- change_intervals(int_dta = x$phid)
        x$f <- as.integer(as.factor(paste0(x$ph_int)))
        x$f <- change_intervals(int_dta = x$f)
