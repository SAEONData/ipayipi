...
#' @param overwrite_sf If `TRUE` then original data (station data) is disgarded in favour of new data. If TRUE both data sets will be evaluated and where there are NA values, a replacement, if available, will be used to replace the NA value. Defaults to FALSE.
...
#' @details
#'  This function searches for all standardised files in the `d3_nomvet_room` and existing 'station files' in the `d4_ipip_room`. Files from the `d3_nomvet_room` that have not been appended to station files will be appended to appropriate stations. Appending is done via the `append_station()`, `append_phen_data()`, and `append_tbls()` functions. The 'aa_nomvet.log' is queried for the list of standardised station files in the 'd3_nomvet_room'.
#'
...
append_station_batch <- function(
  ...
  station_files <- dta_list(
    input_dir = pipe_house$d4_ipip_room, file_ext = sf_ext, prompt = FALSE,
    recurr = FALSE, unwanted = NULL, wanted = NULL
  )
  # list of standardised files in the d3_nomvet_room
  ...
  nom_stations <- nr_lg$stnd_title
  names(nom_stations) <- nr_lg$nomvet_name
  ...
  ups <- future.apply::future_lapply(sf_dtl, function(x) {
    upgraded_stations <- lapply(seq_len(nrow(x)), function(i) {
      ...
      new_data <- readRDS(file.path(pipe_house$d3_nomvet_room,
        x[i][["sf_file"]]
      ))
      new_data$phens$table_name <- new_data$data_summary$table_name[1]
      # get/make station file
      fp <- file.path(pipe_house$d4_ipip_room, paste0(x[i][["sf"]], sf_ext))
      if (!file.exists(fp)) {
