...
#'
#'  A phenomena table or or database is kept in the pipelines 'd2_wait_room' directory as an RDS file ('aa_phentab.rps'). If this file is deleted the `ipayipi::phenomena_sts()` will initiate the recreation of this file. The csv file is only produced for user convienience for editing.
...
phenomena_chk <- function(
  ...
  plist <- ipayipi::dta_list(input_dir = pipe_house$d2_wait_room,
  ...
  t2 <- attempt::attempt(silent = TRUE, as.numeric(max(
    file.info(file.path(pipe_house$d2_wait_room, "aa_phentab.rps"))$mtime
  )))
  c1 <- file.exists(file.path(pipe_house$d2_wait_room, "aa_phentab.rps"))
  ...
  if (file.exists(file.path(pipe_house$d2_wait_room, "aa_phentab.rps"))) {
    phentab <- readRDS(file.path(pipe_house$d2_wait_room, "aa_phentab.rps"))
  ...
  phen_import <- future.apply::future_lapply(slist, function(x) {
    m <- readRDS(file.path(pipe_house$d2_wait_room, x))
  ...
  if (check_phenomena &&
    ...
    if (csv_out) {
      ...
      write.csv(phentab, file.path(pipe_house$d2_wait_room, out_name))
    }
  } else {
