...
header_sts <- function(
  ...
  if (length(slist) == 0) {
    cli::cli_inform(c("!" = paste0(
      "No files read in waiting room ({pipe_house$d2_wait_room})",
  ...
  file_name_dt <- future.apply::future_lapply(seq_along(slist), function(i) {
    ...
    # save m
    saveRDS(m, file.path(pipe_house$d2_wait_room, slist[i]))
