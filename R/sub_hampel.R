...
#' @details
#' The hampel filter has been widely adopted for outlier detection in time-series data. The filter is non-parametric with overall robust behaviour \insertCite{JA_Pearson_2016_JASP}{ipayipi}. Unlike the Tukey filter that has been based on standard deviation from the mean, the Hampel assesses median-absolute deviation (MAD) within the window of interest \insertCite{JA_Pearson_2016_JASP}{ipayipi}. The filter is fully customisable and can be tuned by adjusting the window size (`w`) and the amount of accceptable deviation from the MAD (`d`): if the focal value falls outside the range of the window median, plus minus MAD * d---it is considered an outliler and the median returned.
#' ## `align`:
...
#' ## Implosion sequences:
#' Owing to the failure of the Hampel where MAD approximates 0 when there is little to no deviation in data (i.e., an implosion sequence) \insertCite{JA_Pearson_2016_JASP}{ipayipi}; this function temporariliy adjusts window data by adding a quasi-random, negiable fraction of normally-distributed noise. This noise is approximated with a thousanth of the series MAD around a mean of zero. By adding this noise this customisation avoids the implosion sequence caveat of the Hampel filter.
