#' @title Unpack processing step
#' @description Prepares formatting of a single data processing step.
#' @param dt_n The numeric (integer) order of the processing stage.
#' @param dtp_n A processing stage ('dt_n') contains multiple steps. A stage generally only outputs one processed data table with the specified `time_interval` given at the harvesting stage.
#' @param f_params Paritially evaluated function parameters generated by respective functions' pre-evaluation criteria (_see below_).
#' @param f One of the following functions given as a string. Pre-evaluation for each of these functions is done through the linked functions shown in brackets below:
#'  - "dt_harvest" ([hrv_param_eval()]): function used to harvest data from a station or other source.
#'  - "dt_calc_chain" ([calc_param_eval()]): uses `data.table` chaining to perform calculations, filtering/subsetting, and generate new variables.
#'  - "dt_agg" ([agg_param_eval()]): used to aggregate) data by a specified/default function by a desired `time_interval`.
#'  - "dt_join" ([join_param_eval()]): merges two datasets via overlap, left, right, inner and outer joins.
#'  - *"dt_clean" ([clean_param_eval()]): runs window filters to detect univariate outliers. Imputes with median if requested (Hampel filter).
#'  - *"dt_drift" ([drift_param_eval()]): performs drift corrections based on calibration readings. Under development.
#' @param input_dt The name/k)eyword of the input data table (character string).
#'  If not specified at the harvesting stage, "raw" data imported into a station file will be harvested.
#' @param output_dt The name of the output data table (character string) --- the final table is appended to a station file object.
#' @param time_interval Only has to be specified in the harvesting step. Only one time interval per stage. If the time interval is left as the default `NA` then `time_interval` becomes discontinuous ("discnt").
#' @details Note that the `f_params` argument is parsed to text in this function. It is only evaluated as text again in [dt_process()].
#' @author Paul J. Gordijn
#' @export
p_step <- function(
  dt_n = 1,
  dtp_n = 1,
  f = NULL,
  f_params = NULL,
  input_dt = NA,
  output_dt = NA,
  output_dt_preffix = "dt_",
  output_dt_suffix = NULL,
  time_interval = NA,
  ...
) {
  "%ilike%" <- NULL
  # mandatory args ----
  m <- c("f_params")
  m <- m[sapply(m, function(x) is.null(get(x)))]
  m <- sapply(m[length(m)], function(x) {
    stop(paste0("Missing args: ", paste0(m, collapse = ", ")), call. = FALSE)
  })
  # get argument f
  ff <- list(
    list("dt_harvest", "dt_calc", "dt_agg",
      "dt_join", "dt_clean", "dt_drift",
      "dt_flag"
    ),
    list("hrv_param_eval", "calc_param_eval", "agg_param_eval",
      "join_param_eval", "clean_param_eval", "drift_param_eval",
      "flag_param_eval"
    ),
    list("hrv_params", "calc_params", "agg_params",
      "join_params", "clean_params", "drift_params",
      "flag_params"
    ),
    list("dt_harvest_params", "dt_calc_string", "dt_agg_params",
      "dt_join_params", "dt_clean_params", "dt_drift_params",
      "dt_flag_params"
    )
  )
  f <- ff[[1]][
    ff[[4]] %ilike% class(f_params)[class(f_params) %ilike% "^dt_"][1]
  ][[1]]
  # ensure all 'f's are supported/spelt correctly
  if (
    !any(
      f %in% c(
        "dt_agg", "dt_calc", "dt_clean", "dt_drift", "dt_harvest", "dt_join",
        "dt_flag"
      )
    )
  ) {
    cli::cli_abort("Unrecognised function: {f}")
  }
  # other set up ----
  if (f %in% c("dt_harvest")) {
    if (is.na(time_interval)) time_interval <- "discnt"
    time_interval <- sts_interval_name(time_interval) # time interval
    time_interval <- time_interval$sts_intv
  }

  # auto generate output table name ----
  if (is.na(output_dt) && !is.na(time_interval)) {
    output_dt <- paste0(
      output_dt_preffix, gsub(" ", "_", time_interval), output_dt_suffix
    )
  }

  # special formatting for dt_calc_string ----
  f_params_calc <- f_params[any(class(f_params) %in% "dt_calc_string")]
  f_params_calc <- as.character(f_params_calc)

  # special formatting for dt_harvest ----
  f_params_harvest <- list(
    f_params[any(class(f_params) %in% "dt_harvest_params")]
  )
  f_params_harvest <- f_params_harvest[
    sapply(f_params_harvest, function(x) length(x) != 0)
  ]
  if ("hrv_tbl" %in% names(unlist(f_params_harvest, recursive = TRUE))) {
    input_dt <- unlist(f_params_harvest, recursive = TRUE)[
      names(unlist(f_params_harvest, recursive = TRUE)) %in% "hrv_tbl"
    ]
  } else {
    input_dt <- "raw"
  }
  f_params_harvest <- sapply(f_params_harvest, function(x) {
    deparse1(x, collapse = "")
  })
  f_params_harvest <- sub(
    pattern = "list", replacement = "hrv_param_eval",
    x = f_params_harvest
  )
  # replace list with '.'
  f_params_harvest <- sub(
    pattern = "list['(']", replacement = "\\.\\(",
    x = f_params_harvest
  )
  f_params_harvest <- gsub(
    pattern = ", class = c\\(\"list\", \"dt_harvest_params\"\\))",
    replacement = "", x = f_params_harvest
  )

  # special formatting for dt_join ----
  f_params_join <- list(
    f_params[any(class(f_params) %in% "dt_join_params")]
  )
  f_params_join <- f_params_join[
    sapply(f_params_join, function(x) length(x) != 0)
  ]
  f_params_join <- sapply(f_params_join, function(x) {
    deparse1(x, collapse = "")
  })
  f_params_join <- gsub(
    pattern = "list", replacement = "join_param_eval",
    x = f_params_join
  )

  # special formatting for dt_agg ----
  f_params_agg <- list(
    f_params[any(class(f_params) %in% "dt_agg_params")]
  )
  f_params_agg <- f_params_agg[
    sapply(f_params_agg, function(x) length(x) != 0)
  ]

  # formatting for dt_clean ----
  f_params_clean <- list(
    f_params[any(class(f_params) %in% "dt_clean_params")]
  )
  f_params_clean <- f_params_clean[
    sapply(f_params_clean, function(x) length(x) != 0)
  ]
  f_params_clean <- unlist(f_params_clean, recursive = FALSE)
  f_params_clean <- lapply(f_params_clean, function(x) {
    x <- deparse(x, control = "niceNames", width.cutoff = 500L)
    x <- sub("^expression['(']", "~(", x)
    x
  })

  # formatting for dt_drift ----
  f_params_drift <- list(
    f_params[any(class(f_params) %in% "dt_drift_params")]
  )
  f_params_drift <- f_params_drift[
    sapply(f_params_drift, function(x) length(x) != 0)
  ]
  f_params_drift <- unlist(f_params_drift, recursive = FALSE)
  f_params_drift <- lapply(f_params_drift, function(x) {
    x <- deparse(x, control = "niceNames", width.cutoff = 500L)
    x <- sub("^expression['(']", "~(", x)
    x
  })

  # formatting for dt_flag ----
  f_params_flag <- list(
    f_params[any(class(f_params) %in% "dt_flag_params")]
  )
  f_params_flag <- f_params_flag[
    sapply(f_params_flag, function(x) length(x) != 0)
  ]
  f_params_flag <- unlist(f_params_flag, recursive = FALSE)
  f_params_flag <- lapply(f_params_flag, function(x) {
    if (names(x) %chin% "flag_tbl") x <- bquote(.(x[[1]]))
    x <- deparse(x, control = "niceNames", width.cutoff = 500L)
    x <- sub("^expression['(']", "~(", x)
    x
  })
  # final prep ----
  f_params_text <- list(
    f_params_calc, f_params_harvest, f_params_join,
    f_params_agg, f_params_clean, f_params_drift,
    f_params_flag
  )
  f_params_text <- unlist(f_params_text[
    sapply(f_params_text, function(x) length(x) != 0)
  ])
  if (length(f_params) == 0) {
    f_params_text <- NA_character_
  }
  dt_here <- list(
    dt_n = dt_n,
    dtp_n = dtp_n,
    f = f,
    f_params = f_params_text,
    input_dt = input_dt,
    output_dt = output_dt,
    time_interval = time_interval
  )
  class(dt_here) <- c("list", "pipe_process")
  return(dt_here)
}
