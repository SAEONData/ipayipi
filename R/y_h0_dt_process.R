...
dt_process <- function(
  ...
  mfiles <- c("hrv_dts", "dt_working")
  sf_dta_rm(sfc = sfc, rm = mfiles)

  # purge station dt misc files
  mfiles <- names(sfc)[names(sfc) %ilike% "dt_working|_hrv_tbl_"]
  ...
  lapply(seq_along(pps), function(i) {
    ppsi <- pps[[i]]
    hd <- lapply(unique(ppsi$dtp_n), function(j) {
      sfc <- sf_open_con(sfc = sfc)
      ...
      if (!f %in% c("calc_param_eval", "agg_param_eval", "clean_param_eval",
          "drift_param_eval", "flag_param_eval"
        )
      ) {
        t <- ppsij$f_params[1]
        t <- gsub("\\.\\(", "list\\(", t)
      ...
      o <- append_tbls(original_tbl = oo, new_tbl = o)
      fo <- append_tbls(original_tbl = sfc_f_params, new_tbl = fo)
      ...
      if (!is.null(o$phens_dt)) {
        ...
        opdt <- split.data.frame(o$phens_dt, f = paste(
          o$phens_dt$i, o$phens_dt$orig_tbl_name, o$phens_dt$table_name,
  ...
  sf_log_update(sfc = sfc, log_update = sl)
  sfc <- sf_open_con(pipe_house = pipe_house, station_file = station_file)
  unwanted_tbls <- paste0(c(unwanted_tbls, "_hrv_tbl_"), collapse = "|")
