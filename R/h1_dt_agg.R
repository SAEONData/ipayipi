...
#' @description This function aggregates 'ipayipi' formatted data by custom time intervals.
...
dt_agg <- function(
  ...
  "dttm_fl" <- "phen_name" <-  "agg_f" <- "dt_record_interval" <-
    ...
    "orig_tbl_name" <- "ppsid" <- "p" <- "table_name" <- "tn" <- NULL
  ...
  if (is.null(dta_in) && any(sfcn %ilike% "_hrv_tbl_")) {
    hrv_dta <- sfcn[sfcn %ilike% "_hrv_tbl_"]
    hrv_dta <- hrv_dta[hrv_dta %ilike% paste0(
      unique(f_params$orig_tbl_name
      ), collapse = "|"
    )]
    hrv_dta <- hrv_dta[hrv_dta %ilike% paste0("^", ppsij$dt_n[1], "_.")]
    dta_in <- sf_dta_read(sfc = sfc, tv = hrv_dta)
    names(dta_in) <- sub(".*_hrv_tbl_", "", hrv_dta, fixed = FALSE)
  }
  # filter out unwanted tables by name
  dta_in <- dta_in[
    names(dta_in) %in% c("dt_working", unique(f_params$orig_tbl_name))
  ]
  # get agg parameters ----
  ...
  lapply(seq_along(agg_intv), function(ri) {
    ...
    dta <- dta_in[
      names(dta_in)[
        names(dta_in) %in% c("dt_working", unique(fpi$orig_tbl_name))
      ]
    ]
    ...
    dta <- lapply(names(dta), function(x) {
      if (!x %in% "dt_working") {
        fpii <- fpi[orig_tbl_name %in% x & dt_record_interval == agg_intv[ri]]
      } else {
      ...
      # merge agg f ----
      Reduce(function(...) merge(..., all = TRUE), dtx)
    })
    NA_complex_
  ...
  # clean harvest data ----
  if (!is.null(hrv_dta)) {
    ppsid_hrv <- unique(gsub("_hrv_tbl_?.+", "", hrv_dta))
    hrv_rm <- sfc[
      names(sfc)[names(sfc) %ilike% paste0(ppsid_hrv, "_hrv_tbl_*.")]
    ]
    sf_dta_rm(sfc = sfc, rm = hrv_rm)
  }
