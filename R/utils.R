...
flat_cor_mat <- function(cor_mat) {
  ...
  cor_flat <- cor_flat[order(stnd_title, -r2)]
  return(cor_flat)
}
...
spurge <- function(
  ...
  "%chin%" <- NULL
  chosen <- function() {
    input_dir <- pipe_house$ipip_room
    slist <- dta_list(input_dir = input_dir, baros = TRUE, file_ext = ".ipip",
      wanted = wanted, unwanted = unwanted, prompt = prompt, recurr = TRUE
    )
    cli::cli_inform(c(
      "!" = paste0("Are you sure you want to delete {length(slist)}",
        " file{?s} from the \'ipip_room\'?"
      )
    ))
    ...
    n <- readline(
      prompt =
        "Confirm chosen stations with Y/n:  quit with <q> : "
    )
    ...
    n <- readline(
      prompt =
        "Remove associated files in the \'nomvet_room\'? Y/n:  quit with <q> : "
    )
    # trim white space with regex
    n <- gsub("^\\s+|\\s+$", "", n)
    if (!n %chin% c("Y", "n", "q")) {
    ...
    if (n == "Y") {
      ...
      z <- paste0("^", z)
      input_dir <- pipe_house$nomvet_room
      sapply(z, function(x) {
        nlist <- dta_list(input_dir = input_dir, baros = TRUE,
          wanted = x, unwanted = NULL, prompt = FALSE, recurr = TRUE
        )
...
wpurge <- function(
  ...
  ".SD" <- ":=" <- NULL
  "nas" <- NULL
  input_dir <- pipe_house$wait_room
  # remove imported files
  slist <- dta_list(input_dir = input_dir, baros = TRUE,
    wanted = wanted, unwanted = unwanted, prompt = prompt, recurr = TRUE
  )
...
sf_rename <- function(
  ...
  cli::cli_inform(c("i" = paste0("Renaming is done in the \'wait_room\',",
      " \'nomvet_room\', and \'ipip_room\'."
    ),
  ...
  if (file.exists(file.path(pipe_house$wait_room, "aa_nomtab.rns"))) {
    nt <- readRDS(file.path(pipe_house$wait_room, "aa_nomtab.rns"))
  } else {
    cli::cli_abort(
      "No nomenclature standards in the waiting room{pipe_house$wait_room}"
    )
  }
  ...
  nt1 <- nt[stnd_title %in% old_stnd_title & location %in% old_location &
      station %in% old_station
  ]
  if (nrow(nt1) == 0) {
  ...
  # change nomtab file
  nt[stnd_title %in% old_stnd_title & location %in% old_location &
      station %in% old_station
  ]$location <- new_location
  nt[stnd_title %in% old_stnd_title & location %in% new_location &
      station %in% old_station
  ]$station <- new_station
  nt[stnd_title %in% old_stnd_title & location %in% old_location &
      station %in% new_station
  ...
  nom_csvs <- list.files(path = pipe_house$wait_room,
    pattern = "aa_nomtab.*.csv", full.names = TRUE
  )
  unlink(nom_csvs)

  # save nomtab. rns
  saveRDS(nt, file.path(pipe_house$wait_room, "aa_nomtab.rns"))

  # change files in the nomtab room
  f <- dta_list(input_dir = pipe_house$nomvet_room, file_ext = ".ipi",
    wanted = old_stnd_title
  )
  lapply(seq_along(f), function(i) {
    s <- readRDS(file.path(pipe_house$nomvet_room, f[i]))
    ...
    saveRDS(s,
      file.path(
        pipe_house$nomvet_room,
        gsub(old_stnd_title, new_stnd_title, f[i])
      )
    )
    unlink(file.path(pipe_house$nomvet_room, f[i]))
  })

  # renaming in ipip_room
  f <- dta_list(input_dir = pipe_house$ipip_room, file_ext = ".ipip",
    wanted = old_stnd_title
  )[1]
  s <- readRDS(file.path(pipe_house$ipip_room, f))
  s$data_summary$location <- new_location
  s$data_summary$station <- new_station
  s$data_summary$stnd_title <- new_stnd_title
  ...
  saveRDS(s,
    file.path(
      pipe_house$ipip_room,
      gsub(old_stnd_title, new_stnd_title, f)
    )
  )
  unlink(file.path(pipe_house$ipip_room, f))
