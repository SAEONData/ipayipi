...
sf_dta_chunkr <- function(
  ...
  ci <- ipayipi::sf_dta_chunkr_sub_ri(dta_room = dta_room, indx = indx,
    ...
    rechunk = rechunk, buff_period = buff_period, chunk_v = chunk_v
  )
  chunk_i <- ci$chunk_i
  rit <- ci$rit
  ri <- ci$ri
  ...
  rtb <- ipayipi::chunkr_sub_it(dta_indx = TRUE, mn = dts_min, mx = dts_max,
    buff_period = 0, chunk_i = chunk_i, chunk_v = chunk_v, rit = rit, ri = ri
  )
  ...
  w <- ipayipi::chunkr_inter_chk(dta_room = dta_room, indx = indx,
    i_zeros = i_zeros, chunk_v = chunk_v
  )
  it <- it[!indx %in% w$indx]
  it <- rbind(it, w, fill = TRUE)
  it <- it[,
