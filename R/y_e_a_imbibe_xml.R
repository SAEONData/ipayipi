...
imbibe_xml <- function(
  ...
  # Parse xml data to R
  # Parse xml data to R
  xm <- attempt::try_catch(XML::xmlParse(file_path, encoding = "ISO-8859-1"),
    .e = ~ stop(.x),
    .w = ~ warning(paste0(
  ...
  xm <- attempt::attempt(
    XML::xmlParse(file_path, trim = TRUE, encoding = "ISO-8859-1")
  )
  ...
  phensi <- lapply(phens, function(x) {
    ...
    data.table::data.table(phen_name = zp, units = unit)
  })
  ...
  dttm <- sapply(
    dttree, function(x) lapply(xm[x], XML::xmlValue)
  )
  ...
  dta <- lapply(seq_along(phens$phen_name), function(i) {
