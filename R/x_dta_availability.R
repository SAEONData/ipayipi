...
#' @param input_dir Defaults to the working diretory. If the `pipe_house` arument is provided then the working directory defaults to `pipe_house$d4_ipip_room`.
...
#' @param station_ext Required argument. File extension of the station files in the pipe_house directory 'd4_ipip_room'.
...
dta_availability <- function(
  ...
  if (is.null(pipe_house)) pipe_house <- list(d4_ipip_room = input_dir)

  slist <- ipayipi::dta_list(input_dir = pipe_house$d4_ipip_room,
