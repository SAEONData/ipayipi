...
read_phentab_csv <- function(
  ...
  if (is.null(file)) {
    phenlist <- ipayipi::dta_list(input_dir = pipe_house$d2_wait_room,
      file_ext = ".csv", wanted = "aa_phentab"
    )
    if (length(phenlist) < 1) {
      cli::cli_abort(c(
        "!" = "There is no aa_phentab csv file in the d2_wait_room!",
  ...
  phentab <- data.table::fread(file.path(pipe_house$d2_wait_room, file),
    header = TRUE
  )
