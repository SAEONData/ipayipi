  ...
  if (length(slist) == 0) {
    ...
    emmasf <- sapply(slist, FUN = function(emm) {
      ...
      if (nrow(rlog) > 0 & nrow(sol_temp$log_t) > 0) {
        ...
        dt <- data.table(Date_time = as.POSIXct(c(min(drifting$Date_time),
        ...
        if (length(x) > 0) {
          ...
          drifting_ttt$l <- c(c(drifting_ttt$j[c(1:(nrow(drifting_ttt) - 1))]
            - drifting_ttt$j[c(2:(nrow(drifting_ttt)))]) * -1, 1)
          drifting_ttt$l[1] <- 2
      ...
      naarbs <- lapply(seq_len(segs_n), function(z) {
          ...
          cr_msg <- padr(core_message = paste0("seg ",
            z, ": ", tab$Date_time[1], " --> ", tab$Date_time[nrow(tab)],
            collapse = ""), pad_char = c(" "), pad_extras =
            c("|", "", "", "|"), force_extras = TRUE,
            justf = c(-1, 2), wdth = 59)
          ...
          if (nrow(tab_ts) > 3 & should_i == TRUE) {
            ...
            if (is.na(rule)) {
              msg <- paste0(" Dflt rule: ",
                "hf_", w_size, "_", mad_dev, " |")
      ...
      for (i in seq_len(nrow(sol_temp$log_t_man_out))) {
        ...
        sol_temp$log_t[
          which(sol_temp$log_t$Date_time >= start &
            sol_temp$log_t$Date_time <= end), "bt_Outlier"] <- TRUE
        sol_temp$log_t[
          which(sol_temp$log_t$Date_time >= start &
            sol_temp$log_t$Date_time <= end), "t_bt_level_m"] <- NA
      }
      saveRDS(sol_temp, file.path(input_dir, emm))
    }
    cr_msg <- padr(core_message = paste0(" naartjies consumed ",
      collapse = ""), pad_char = "=", wdth = 80,
      pad_extras = c("|", "", "", "|"), force_extras = FALSE, justf = c(0, 0))
    message(cr_msg)
  }
  )
