...
#' @param d1_source_room **1**. 'Room' from which raw-data files are imported into the pipeline. If left `NULL` a directory named 'd1_source_room' will be created in the pipe_house working directory.
#' @param d2_wait_room **2**. Directory into which raw-data moves to from the `d1_source_room`. This is also the room where standardisation of raw-data files begins.
#' @param d3_nomvet_room **3**. 'Room' into which imbibed, standardised raw-data is stored.
#' @param d4_ipip_room **4**. 'Room' where 'station files' are housed. Station files consist of appended, standardised raw-data files---pulled from the `d3_nomvet_room`---that have been, or are ready for further processing.
#' @param d5_dta_out Directory housing select data products from the pipeline; typically in 'csv' file format.
#' @param reports Intended to house 'rmarkdown' reports used for inspection and analysis of data.
#' @param d0_raw_room 'Room' wherein raw-data files from the `d1_source_room` are systematically archived. If this is set to `NULL` then raw-data files will not be archived by 'ipayipi'.
...
#' 2. *d1_source_room*: The source directory where raw data are harvested from.
#' 3. *d2_wait_room*: A staging directory where 'raw data' are standardised before being transferred/archived in the,
#' 4. *d3_nomvet_room*: Folder housing standardised input data from multiple stations. By appending these standarised files station records (databases) are compiled. 
#' 5. *d4_ipip_room*: The directory that compiles standardised data from the `d3_nomvet_room` for each station. Station files can be further processed and exported into other formats from here.
#' 6. *d5_dta_out*: Data exports typically generated using `ipayipi2csv()`, `dta_flat_pull()` or, `dta_flat_pull_discnt()`.
#' 6. *d0_raw_room*:  If defined [imbibe_raw_batch()] can harvest (copy or cut---see funtion documentation) and archive 'raw data' files from the `d1_source_room` and will archive them in the `d0_raw_room` in monthly folders.
#' \cr
...
ipip_house <- function(
  ...
  # dir 'names'
  dirs <- list("r", "d1_source_room", "d2_wait_room", "d3_nomvet_room",
    "d4_ipip_room", "d5_dta_out", "reports", "d0_raw_room"
  )
  ...
  names(dirs) <- c("r", "d1_source_room", "d2_wait_room", "d3_nomvet_room",
    "d4_ipip_room", "d5_dta_out", "reports", "d0_raw_room"
  )
  ...
  if (verbose) {
    cli::cli_inform(c(
      ...
      " " = paste0("Begin processing data by housing new, raw-flat files in",
        " the {.var d1_source_room}.",
      ), "i" = paste0("From the {.var d1_source_room} data is copied to the ",
        "{.var d2_wait_room} using the function:"
