...
dt_harvest <- function(
  ...
  if (station_file == unique(f_params$hsf_station)[1]) {
    ...
    hsfc <- attempt::try_catch(
      expr = sf_open_con(station_file = unique(f_params$hsf_station)[1],
        chunk_v = chunk_v
      ), .w = ~stop
    )
  }
  ...
  log_update <- lapply(hsf_names, function(x) {
    h <- sf_dta_read(sfc = hsfc, tv = x)
    # unchunked files ----
    ...
    lg_tbl <- data.table::data.table(
      tbl_n = paste0(ppsid, "_hsf_table_", x),
