...
#' # 'tab' being table.
#' ## set variables
#' wait_room <- "./temp" # directory (temporary)
#' csv_file <- "./nomtab_20210415_09h42.csv"
#'
#' ## read in the nomenclature table
#' read_nomtab_csv(wait_room = wait_room, file = csv_file)
rain_read_nomtab_csv <- function(
  wait_room = NULL,
  file = NULL,
  ...) {
  if (is.null(file)) {
  ...
  if (cans == 0) {
    nomtab <- transform(nomtab,
      uz_import_file_name = as.character(nomtab$uz_import_file_name),
      uz_location_station = as.character(nomtab$uz_location_station),
      uz_title = as.character(nomtab$uz_title),
      location = as.character(nomtab$location),
      station = as.character(nomtab$station),
      stnd_title = as.character(nomtab$stnd_title),
      instrument = as.character(nomtab$instrument),
      logger_sn = as.character(nomtab$logger_sn),
      start_dt = as.POSIXct(nomtab$start_dt),
      end_dt = as.POSIXct(nomtab$end_dt)
    )
