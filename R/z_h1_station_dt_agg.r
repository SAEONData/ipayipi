...
dt_agg <- function(
  ...
  "dttm_fl" <- "phen_name" <-  "agg_f" <- "dt_record_interval" <-
    ...
    "xdt2" <- "orig_table_name" <- "ppsid" <- "d1" <- "d2" <- NULL
  ...
  gap_tbl <- ipayipi::sf_read(sfc = sfc, tv = "gaps", tmp = TRUE)[["gaps"]]
  ...
  agg_dts <- lapply(seq_along(agg_intv), function(ri) {
    ...
    dta_ex <- ipayipi::sf_dta_read(sfc = sfc, tv = names(agg_intv)[ri],
      tmp = TRUE
    )
