...
#' @return A phenomena table that contains the start and end dates of each phenomena and the respective data source identification numbers of the data. Also, the appended data.
#' @export
...
append_phen_data <- function(
  ...
  if (is.character(station_file)) {
    ...
    # read_dta
    sf_eindx <- sf_dta_read(sfc = sfc, start_dttm = nd_min,
      end_dttm = nd_max, return_dta = FALSE, tv = tn
    )[[tn]]
  ...
  lg_update <- lapply(list(ndno1, ndno2), function(dtx) {
    sl <- sf_dta_wr(dta_room = sfc[tn], dta = dtx, tn = tn, ri = ri, rit = rit,
      chunk_v = chunk_v
    )
    return(sl)
  })
  sf_log_update(sfc = sfc, log_update = lg_update)

  # add in overlap data
  fs <- dta_list(input_dir = ovlap$dta_room, unwanted = "aindxr")
  sl <- lapply(fs, function(x) {
    dta <- readRDS(file.path(ovlap$dta_room, x))
    sl <- sf_dta_wr(dta_room = sfc[tn], dta = dta, tn = tn, ri = ri, rit = rit,
      chunk_v = chunk_v
    )
    return(sl)
  })
