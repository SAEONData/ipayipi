...
#'  1. _plonk_. Simple row binding of columns (rbind) with `fill = NA`. The 'plonk doesn't use join keys etc.
#' __Keys__
...
#' @param fuzzy A numeric (seconds) vector of length one or two. Or character strings that can be convered to 'lubridate' durations.
...
mhlanga <- function(
  ...
  # plonk ----
  # plonk and return xy here ...
  if (join == "plonk") return(rbind(x_tbl, y_tbl, fill = TRUE))
  ...
  fuzzy <- sapply(fuzzy, function(x) {
    if (is.character(x)) x <- lubridate::as.duration(x) else x
    return(x)
  })
  if (length(fuzzy) == 0) fuzzy <- NULL
  # check keys
  if (!is.null(fuzzy)) {
  ...
  if (any(rollends != FALSE)) {
    on <- paste0(on, " , rollends=c(", as.character(rollends[1]), ", ",
      as.character(rollends[length(rollends)]), ")", collapse = ""
    )
  }

  if (is.character(x_tbl$date_time) && any(x_key %chin% "date_time")) {
    x_tbl$date_time <- lubridate::parse_date_time2(
      x = x_tbl$date_time, orders = dt_format, tz = dt_tz
    )
  }
  if (is.character(y_tbl$date_time) && any(x_key %chin% "date_time")) {
    y_tbl$date_time <- lubridate::parse_date_time2(
      x = y_tbl$date_time, orders = dt_format, tz = dt_tz
    )
  }
  ...
  if (join == "right_join") {
    ...
    if (xtra_v) cli::cli_inform(c(
      "*" = "Right join data.table syntax:", dsyn
    ))
    xy <- eval(parse(text = dsyn))
  }
  ...
  if (join == "inner_join") {
    ...
    if (xtra_v) cli::cli_inform(c(
      "*" = "Inner join data.table syntax:", dsyn
    ))
    xy <- eval(parse(text = dsyn))
  }
  ...
  if (join == "full_join" && !any(time_seq[1], time_seq[2])) {
    ...
    if (xtra_v) cli::cli_inform(c(
      "*" = "Full join data.table syntax:", j_txt
    ))
