...
#'  1. _plonk_. Simple rbind(fill = TRUE) (_see_ [base::rbind()]).
...
dt_join <- function(
  ...
  if (length(wrk_dta) > 0) {
    ...
    x_tbl_n <- hrv_dta[length(hrv_dta) - 1]
    x_tbl <- ipayipi::sf_dta_read(sfc = sfc, tv = x_tbl_n)
    gx <- x_tbl[[x_tbl_n]]$gaps
  }
  x_class <- class(x_tbl[[1]])
  if (length(hrv_dta) > 0) {
    y_tbl_n <- hrv_dta[length(hrv_dta)]
    y_tbl <- ipayipi::sf_dta_read(sfc = sfc, tv = y_tbl_n)
  ...
  if (xtra_v) {
    cli::cli_inform(c(" " = "Pre-join x & y data :"))
    cli::cli_inform(c(" " = "|> x table "))
    print(head(x_tbl))
  ...
  if (xtra_v) {
    cli::cli_inform(c(" " = "Post-join data :"))
    cli::cli_inform(c(" " = "|> head "))
    print(head(dt_working))
    cli::cli_inform(c(" " = "|> tail "))
    print(tail(dt_working))
    cli::cli_inform(c(" " = "data class = {class(dt_working)}"))
  }
  # dttm for ppsij
  if (all(
  ...
  # if (length(hrv_dta) > 0) {
  #   lapply(sfc[names(sfc) %in% hrv_dta], function(x) {
  #     unlink(x, recursive = TRUE)
  #   })
  # }
  # unlink(file.path(dirname(sfc)[1], "dt_working"), recursive = TRUE)

  # sort out classes -- avoids transfer of ipip-sf_rds to x table from
  # y-table if date_time is present---x table passes on class
  if ("date_time" %in% names(dt_working) && !"ipip-sf_rds" %in% x_class) {
    class(dt_working) <- c(class(dt_working), "ipip-sf_chnk")
    class(dt_working) <- class(dt_working)[
      !class(dt_working) %in% "ipip-sf_rds"
    ]
  }
  sf_dta_rm(sfc = sfc, rm = "dt_working")
  sl <- sf_dta_wr(dta_room = file.path(dirname(sfc)[1], "dt_working"),
    dta = dt_working, tn = "dt_working",
    ri = ppsij$time_interval[1], chunk_v = chunk_v
  )
