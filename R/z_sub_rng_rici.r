...
#' @description A predefined function to return the row and column indicies
...
#' @param r_fx Single integer value corresponding to the row number wherein
#'  lie all row indicies of interest.
#' @param c_fx Single integer value corresponding to the column number wherein
#'  lie all column indicies of interest.
#' @param c_fx_start If the `c_rng` or `c_fx` parameters are not supplied the
#'  function needs the start (`c_fx_start`) and end (`c_fx_end`) of the column
#'  indicies where phenomena are stored.
#' @param c_fx_end If the `c_rng` or `c_fx` parameters are not supplied the
#'  function needs the start (`c_fx_start`) and end (`c_fx_end`) of the column
#'  indicies where phenomena are stored. `c_fx_end` can be extracted from the
#'  supplied file if this option is set to "extract".
#' @param string_extract If these options are specified the function will store
...
#'    1. `rng_pattern` A string used to search the range used to identify the
#'     position of the substring of interest to be extracted.
#'    1. `rel_start` The starting position (integer) of the string relative to
#'      the begining of the `rng_pattern`.
#'    1. `rel_end` The end position (integer) of the string relative to the
#'      begining of the `rng_pattern`.
#' @param setup_name The name of the item/data found at the defined index. A
#'  character string.
#' @details This is an internal function not meant to be used as stand alone
#'  feature.
#' @return A list of length two containing interger indicies for a row and
#'  column, respectively.
#' @export
