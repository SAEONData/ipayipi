...
dt_clean <- function(
  ...
  if (xtra_v) cli::cli_inform(c("i" = "{nrow(zouts)} outliers detected."))
  f <- rbind(f, zouts)
  ...
  sf_log_update(sfc = sfc, log_update = sl)
  ppsij <- ppsij[, ":="(
    start_dttm = dta_in_o[[1]][[hrv_dta]]$indx$mn,
    end_dttm = dta_in_o[[1]][[hrv_dta]]$indx$mx
  )]
