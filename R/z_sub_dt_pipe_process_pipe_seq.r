...
#' @param output_dt_preffix Defaults to "_dt". Must only be changed if
#'  absolutely necessary.
#' @param output_dt_suffix Defaults to "_n". Not yet fully implemented.
#' @return Data processing pipeline in the format of a `data.table` of
#'  class "pipe_seq".
#' @examples
...
pipe_seq <- function(
  ...
  "pijz" <- "dtp_n" <- "dt_n" <- "f" <- "time_interval" <-
    "pijt" <- "pipe_seq_dt" <- "output_dt" <- NULL
  ...
  pu <- unique(p$output_dt)
  if (length(pu[!is.na(pu)]) < max(p$dt_n)) {
    m <- paste0("Each stage \'dt_n\' requires a unique \'output_dt\'.")
    stop(m)
  }
