...
#' @param pipe_house If provided the 'd4_ipip_room' will be used as the input directory. _See_ `ipip_house()`.
...
plot_bar_agg <- function(
  ...
  unwanted <- paste("*.ipr$|*.ipi$|*.iph$|*.xls$|*.rps|*.rns$|*.ods$|*.doc$",
    "d2_wait_room|d3_nomvet_room|*.r$|*.rmds$", unwanted, sep = "|"
  )
  ...
  p <- ggplot2::ggplot(dts, ggplot2::aes(
    ...
    ggplot2::geom_point(
      ggplot2::aes(y = !!as.name(phen_name)),
