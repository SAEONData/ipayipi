...
#' @description Moves files from the 'wait_room' to the 'nomvet_room'. Avoids
#'  duplicating files in the 'nomvet_room' and ensures file names are
#'  unique. Files will be named using their station and table names, their date
#'  coverage, and an integer (that is unique per data file & respective
#'  date-time stamps). The nomvet directory contains files which have been
#'  standardised for the 'ipayipi' data pipeline.
...
transfer_sts_files <- function(
  ...
  ...) {
    ...
    merge_dt <- lapply(slist, function(x) {
      m <- readRDS(file.path(wait_room, x))
    ...
    wr_lg <- ipayipi::ipayipi_data_log(log_dir = wait_room,
      file_ext = file_ext_in)
    wr_lg <- wr_lg$log
    nr_lg <- ipayipi::ipayipi_data_log(log_dir = nomvet_room,
      file_ext = file_ext_out)
    nr_lg <- nr_lg$log
    to_transfer <- data.table::fsetdiff(
      wr_lg, data.table::fintersect(wr_lg, nr_lg))
    ...
    bnames <- lapply(seq_len(nrow(to_transfer)), function(x) {
      i <- 1
      while (file.exists(
          file.path(nomvet_room, paste0(to_transfer$basename[x]
            , "__", i, file_ext_out)))) {
        i <- i + 1
      }
    ...
    deleted_files <- lapply(seq_len(nrow(to_transfer)), function(x) {
      file.remove(file.path(wait_room, to_transfer$input_file[x]))
      invisible(to_transfer$input_file[x])
    })
    rm(deleted_files)
    cr_msg <- padr(core_message = "", wdth = 80, pad_char = "=",
