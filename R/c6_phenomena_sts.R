...
#' @param external_phentab File path to phenomena database. By default the function will search in the 'pipe_house' 'd2_wait_room' for a phenomena database ('phentab'). If `external_phentab` is provided this option will override the 'pipe_house' default enabling access to another pipelines phenomena database.
...
#'  When unrecognised synonyms are introduced (e.g., after a change in logger program setup, or 'new' variable from an additional station sensor) this function creates a csv table in the `d2_wait_room` that should be edited to maintain a phenomena synonym database. Unstandardised phenomena metadata (columns) are preffixed by 'uz_'._ These unstandardised columns must not be edited by the user: standardised phenomena information can be added by replacing corresponding `NA` values with information. The required fields, marked with an asterix (above), must be described.
#'
...
phenomena_sts <- function(
  ...
  slist <- ipayipi::dta_list(input_dir = pipe_house$d2_wait_room, file_ext =
      file_ext_in, prompt = prompt, recurr = recurr, unwanted = unwanted,
    wanted = wanted
  )
  if (length(slist) == 0) {
    cli::cli_inform(c(paste0(
      "No files in waiting room ({pipe_house$d2_wait_room})",
  ...
  fupdates <- future.apply::future_lapply(seq_along(slist), function(i) {
    ...
    m <- readRDS(file.path(pipe_house$d2_wait_room, slist[i]))
    ...
    if (any(
      ...
      fn <- file.path(pipe_house$d2_wait_room, slist[i])
    ...
    new_names <- sapply(names(m$raw_data)[
      ...
      phen_new[uz_phen_name == x]$phen_name[1]
    })
    ...
    if (length(phen_new[var_type == "posix"]$phen_name) > 0) {
      ...
      m$raw_data[, (phen_new[var_type == "posix"]$phen_name) :=
        lapply(.SD, function(x) {
          lubridate::parse_date_time(x, tz = dt_tz, orders = dt_format)
        }),
    ...
    old_fn <- file.path(pipe_house$d2_wait_room, slist[[i]])
    fn <- file.path(pipe_house$d2_wait_room,
  ...
  if (nrow(tbl_update) > 0) {
    fn <- file.path(pipe_house$d2_wait_room, basename(tbl_update$fn))
    old_fn <- file.path(pipe_house$d2_wait_room, basename(tbl_update$old_fn))
  ...
  if (verbose || xtra_v) {
    ...
    cli::cli_inform(c(
      ...
      ">" = paste0("Use {.var transfer_sts_files()} to move \'ipi\' files to ",
        "the {.var d3_nomvet_room}."
      ),
      " " = "This will clean up the {.var d2_wait_room}."
    ))
  }
