...
gw_xle_to_R <- function(
    ...
    # Logger info
    xle_fi <-
        data.table(XML::xmlToDataFrame(testxml["//Body_xle/Instrument_info"],
        stringsAsFactors = FALSE
        ))
    xle_fi[xle_fi == ""] <- NA
    fds <- c(
    ...
    if ("Schedule" %in% c(colnames(xle_fi))) {
        ...
        as.POSIXct(as.character(xle_fi$Schedule)),
    ...
    dtb <- as.data.frame.list(dtb, stringsAsFactors = F)
    xle_LoggerHeader <- data.table::data.table(
        ...
        Start_time = as.POSIXct(as.character(dtb$Start_time)),
        Stop_time = as.POSIXct(as.character(dtb$Stop_time)),
    ...
    xle_phenomena <- sapply(params, function(x) {
        ...
        if (length(p) > 1) {
            p$Start <- as.POSIXct(as.character(st_dt))
            p$End <- as.POSIXct(as.character(ed_dt))
        }
        if (nrow(p) > 0) {
    ...
    if (length(nayyay) != 0) {
        log_data[, nayyay] <- log_data[, nayyay] * 6.89476
    ...
    if (length(nayyay) != 0) {
        log_data[, nayyay] <- log_data[, nayyay] / 100
        ...
        xle_phenomena$level_m$Vars == "Offset",
        ][, "Val"] <-
    ...
    if (length(transfer_log_vet) > 0) {
        ...
        if (sum(grep(
            pattern = basename(xlefile),
            x = transfer_log$xle_file
        )) > 0) {
            overwrite <- FALSE
        }
        ...
        saveRDS(data.table(
            ...
            to = character()
            ), file.path(output_dir, "transfer_log.rds")
            )
