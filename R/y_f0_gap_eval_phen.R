...
phen_gaps <- function(
  ...
  dts <- sf_dta_read(sfc = sfc, tv = names(tbl_n))
  # filter out unwanted phens, focus on wanted phens
  ...
  phen_ds <- sf_dta_read(sfc = sfc, tv = "phen_data_summary"
  )[["phen_data_summary"]]
  # read station data
  phen_gaps <- lapply(seq_along(names(dts)), function(i) {
    dta <- dt_dta_open(dta_link = dts[names(dts)[i]])
    ...
    gaps <- sf_dta_read(sfc = sfc, tv = "gaps"
