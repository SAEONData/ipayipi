...
hsf_param_eval <- function(
  ...
  "phen_name" <- "table_name" <- "input_dt" <- "phen_syn" <- "dt_n" <-
    "dtp_n" <- "output_dt" <- "orig_table_name" <- "dt_record_interval" <-
    "phen_gap" <- NULL
  ...
  if (full_eval != TRUE) {
    ...
    if (!is.null(hsf_station)) {
      hsfn <- if (!file.exists(hsf_station)) {
    ...
    if (basename(hsfn) != basename(station_file)) {
      hsfc <- attempt::try_catch(
        sf_open_con(station_file = hsfn, tv = hsf_table, chunk_v = chunk_v)
      )
    } else {
      hsfc <- sfc
    }
    hsf_summary <- sf_dta_read(
      sfc = hsfc, tv = c("data_summary", "phens", "phens_dt")
    )
    ...
    if (!any(sapply(phen_tabs, function(x) any(x$table_name %in% hsf_table))) &&
      ...
      # open table
      ht <- ipayipi::sf_dta_read(sfc = hsfc, tv = hsf_table)
      ...
      # )
      # set record interval as defined by hsf
