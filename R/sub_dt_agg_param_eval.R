...
#'  _Note_:
#'   - phenomena with variable types (`var_type`) character (`chr`), factor (`fac`), and POSIXct ("posix") are not processed by this function.
...
agg_param_eval <- function(
  ...
  if (!full_eval) { # partial evaluation
    ...
    ][!var_type %in% c("chr", "fac", "posix") # not aggr character/factors/posix
    ...
    agg_details <- data.table::data.table(ppsid = NA_character_,
      ...
      orig_tbl_name = NA_character_, table_name = NA_character_
    )[0, ]
    ...
    rps <- lapply(agg_intervals, function(x) {
      ...
      gaprow <- transform(gaprow, phen_name = "gap", units = "logi", measure =
          "smp", var_type = "logi", orig_tbl_name = "gaps",
      ...
      if (nrow(nphen_dt) > 0) {
        ...
        np <- np[paste(np$orig_tbl_name, np$phen_out_name) %in%
            paste(nphen_dt$orig_tbl_name, nphen_dt$phen_name)
        ]
      ...
      list(phens_dt_new = nphen_dt, p = np)
    })
