...
#'
#' ## 'Archiving' raw data
#'  Files brought into the 'd2_wait_room' are only housed there temporarily. In order to archive these raw data files a 'd0_raw_room' directory must be provided in the 'pipe_house' object (_see_ [ipip_house()]). Files will be archived in structured directories in the 'd0_raw_room' named by the last year and month in their respective date time records. Original  file names are maintained, and have a suffix with a unique integer plus the date and time of which they were archived. N.B. Files in the source directory (`source_dir`) are only deleted when `wipe_source` is set to `TRUE`.
#'
#' ## Reading flat files
...
imbibe_raw_batch <- function(
  ...
  unwanted <- paste0("['.']ipr|['.']ipi|['.']iph|['.']xls|['.']rps",
    "['.']rns|['.']ods|['.']doc|['.']md|d2_wait_room", unwanted,
    collapse = "|"
  )
  ...
  if (length(slist) == 0) {
    exit_m <- cli::cli_inform(c(
      "No '{file_ext_in}' files to imbibe in the pipeline {.var d2_wait_room}.",
      "i" = "Pipe house 'd2_wait_room' path: {pipe_house$d2_wait_room}.",
  ...
  file_name_dt <- future.apply::future_lapply(seq_along(slist), function(i) {
    ...
    fp <- file.path(pipe_house$d2_wait_room, slist[i])
    ...
    if (!fl$err) {
      fn_htmp <- tempfile(pattern = "raw_",
        tmpdir = file.path(getOption("chunk_dir"), "wait_room_tmp")
      )
      ...
      if (!file.exists(dirname(fn_htmp))) {
        dir.create(dirname(fn_htmp), recursive = TRUE)
      }
  ...
  # requires non-null d0_raw_room
  if (!is.null(pipe_house$d0_raw_room)) {
    arc_dir <- file.path(pipe_house$d0_raw_room, fn_dt_arc$yr_mon_end)
    lapply(unique(arc_dir[!file.exists(arc_dir)]), function(x) {
